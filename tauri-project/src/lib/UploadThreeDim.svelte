      ...
      <div class="grid gap-8">
        <div class="bg-white dark:bg-gray-950 rounded-lg shadow-sm p-6">
          ...
          <div id="drop-area" class="bg-gray-100 dark:bg-gray-800 rounded-lg p-6 border border-gray-200 dark:border-gray-700 flex flex-col items-center justify-center space-y-4 cursor-pointer transition-colors hover:bg-gray-200 dark:hover:bg-gray-750">
            ...
            <input class="h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 hidden" multiple="" type="file" />
          </div>
        </div>
        <div class="bg-white dark:bg-gray-950 rounded-lg shadow-sm p-6">
          <div class="flex items-center justify-between mb-4">
  ...
  <script>
      ...
      fetch(url, {
        ...
        .then((data) => {
          ...
          if (data.filename) {
            ...
            reloadPage();
            // addFileToListNew(data.filename);
            // reloadPageSilently();
          }
        })
        .catch(console.error);
    ...
    function deleteFile(filename) {
      fetch(`/delete/${filename}`, { method: "POST" })
        .then((response) => {
          if (response.ok) {
            let fileElement = document.getElementById(filename);
