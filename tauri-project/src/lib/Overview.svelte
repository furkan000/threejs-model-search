<script>
  import BackIcon from "./BackIcon.svelte";
  import AnnotateIcon from "./AnnotateIcon.svelte";
  import ThreeDimIcon from "./ThreeDimIcon.svelte";
  import UploadIcon from "./Icons/UploadIcon.svelte";
</script>

<!--
// v0 by Vercel.
// https://v0.dev/t/L8nO37MW4qo
-->

<main class="flex flex-col h-screen">
  <header class="relative flex items-center justify-center bg-gray-900 px-6 py-4 dark:bg-gray-950">
    <a class="disabled absolute left-6 flex items-center gap-3" href="#">
      <BackIcon />
      <span class="text-xl font-semibold text-gray-50">Back</span>
    </a>
    <h1 class="text-xl font-semibold text-gray-50">Authoring Tool</h1>
    <div class="w-6 h-6"></div>
  </header>

  <body class="flex h-full w-full flex-col items-center justify-center gap-16 bg-gray-100 px-6 py-16 dark:bg-gray-950">
    <div class="container mx-auto grid max-w-6xl grid-cols-1 gap-16 md:grid-cols-4">
      <a class="group flex flex-col items-center justify-center gap-8 rounded-xl bg-white p-16 text-center shadow-sm transition-colors hover:bg-gray-50 dark:bg-gray-900 dark:hover:bg-gray-800" href="/upload-rag">
        <div class="flex h-24 w-24 items-center justify-center rounded-full bg-blue-500 text-white group-hover:bg-blue-600">
          <!-- <UploadIcon /> -->
          <svg class="h-12 w-12" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z" /><path d="M14 2v4a2 2 0 0 0 2 2h4" /><path d="M10 9H8" /><path d="M16 13H8" /><path d="M16 17H8" /></svg>
        </div>
        <div>
          <h3 class="text-2xl font-semibold tracking-tight">Upload Document</h3>
          <p class="mt-2 text-gray-500 dark:text-gray-400 text-lg">Augment your agent with information.</p>
        </div>
      </a>
      <a class="group flex flex-col items-center justify-center gap-8 rounded-xl bg-white p-16 text-center shadow-sm transition-colors hover:bg-gray-50 dark:bg-gray-900 dark:hover:bg-gray-800" href="/upload-three-dim">
        <div class="flex h-24 w-24 items-center justify-center rounded-full bg-green-500 text-white group-hover:bg-green-600">
          <ThreeDimIcon />
        </div>
        <div>
          <h3 class="text-2xl font-semibold tracking-tight">Upload 3D Model</h3>
          <p class="mt-2 text-gray-500 dark:text-gray-400 text-lg">Upload models for annotation and import.</p>
        </div>
      </a>
      <a class="group flex flex-col items-center justify-center gap-8 rounded-xl bg-white p-16 text-center shadow-sm transition-colors hover:bg-gray-50 dark:bg-gray-900 dark:hover:bg-gray-800" href="/editor">
        <div class="flex h-24 w-24 items-center justify-center rounded-full bg-yellow-500 text-white group-hover:bg-yellow-600">
          <AnnotateIcon />
        </div>
        <div>
          <h3 class="text-2xl font-semibold tracking-tight">Annotate Model</h3>
          <p class="mt-2 text-gray-500 dark:text-gray-400 text-lg">Annotate models for better interactions.</p>
        </div>
      </a>

      <a class="group flex flex-col items-center justify-center gap-8 rounded-xl bg-white p-16 text-center shadow-sm transition-colors hover:bg-gray-50 dark:bg-gray-900 dark:hover:bg-gray-800" href="/search">
        <div class="flex h-24 w-24 items-center justify-center rounded-full bg-cyan-500 text-white group-hover:bg-cyan-600">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-12 w-12"><circle cx="11" cy="11" r="8" /><path d="m21 21-4.3-4.3" /></svg>
        </div>
        <div>
          <h3 class="text-2xl font-semibold tracking-tight">Search 3D Models</h3>
          <p class="mt-2 text-gray-500 dark:text-gray-400 text-lg">Find 3D models based on AI generated description.</p>
        </div>
      </a>
    </div>
  </body>
</main>

<style>
  .disabled {
    pointer-events: none;
    opacity: 0.5;
  }
</style>
